...
try {
  xl = window.top.location.pathname === "/sigma/rx.html";
} catch {
  try {
    xl = window.parent.location.pathname === "/sigma/rx.html";   
  } catch {
    xl = false;
  }
}
...
if (form && input) {
  form.addEventListener("submit", async event => {
    ...
    try {
      ...
      else processUrl(input.value, "/sigma/rx.html");
    } catch {
      processUrl(input.value, "/sigma/rx.html");
    }
  });
}
...
function go(value) {
  processUrl(value, "/sigma/rx.html");
}
